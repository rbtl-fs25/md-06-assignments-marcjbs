...
2.  What's the `tidyverse` Package? Describe in maximum two sentences below.
    -   tidyverse is a R package/library containing several data manipulation and analysis tools
...
1.  Starting with the `waste_data_long_mean` object, execute the code and observe the output in the Console. Would you publish this table in this format in a report?
    -   I would output this data set in several bar diagrams as it would make the data more readable, or make the table wide, as this format isn't helping readablity for humans. This format is meant for data manipulation and analysis.
...
2.  Execute the code and observe the output in the Console. Would you publish this table in a report in this format?
    ...
    -   This table is much more readable, so I would publish the table like this in a report.
